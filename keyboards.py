...
def main_menu(is_admin=False):
    ...
    kb.add_button("Помощь", color=VkKeyboardColor.SECONDARY) # Новая кнопка Помощь
    if is_admin:
        kb.add_line()
...
# НОВЫЕ КЛАВИАТУРЫ ДЛЯ ПОМОЩИ
def help_menu(is_admin=False):
    kb = VkKeyboard(one_time=True)
    kb.add_button("Помощь для клиента", color=VkKeyboardColor.PRIMARY)
    if is_admin:
        kb.add_line()
        kb.add_button("Помощь для админа", color=VkKeyboardColor.NEGATIVE)
    kb.add_line()
    kb.add_button("В главное меню", color=VkKeyboardColor.SECONDARY) # Возврат в основное меню
    return kb.get_keyboard()

def back_to_help_menu_keyboard():
    kb = VkKeyboard(one_time=True)
    kb.add_button("Назад к помощи", color=VkKeyboardColor.SECONDARY)
    return kb.get_keyboard()
